...
<script type="text/javascript">
	...
	var user = "{\"name\": \"TODAY\", \"id\" : 20 ,\"password\": \"passwd\"}";
...
//		ajax(contextPath + "/body/list", "{\"users\":" + JSON.stringify(users) + "}", "POST", true, function(data) {
		ajax(contextPath + "/body/list", JSON.stringify(users), "POST", true, function(data) {
			$.each(data, function(index, user) {
				$("#data").append(user.description + "</br>");
				$("#data").append(user.id + "</br>");
				$("#data").append(user.name + "</br>");
			});
		}, function() {
			alert("error");
		});
